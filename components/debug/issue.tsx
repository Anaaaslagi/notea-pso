...
// 👇 Gunakan typing yang eksplisit agar JSX mengenali komponen
const StyledAccordion = withStyles({
  ...
  expanded: {},
})(MuiAccordion) as ComponentType<any>;
...
const StyledAccordionSummary = withStyles({
  ...
  expanded: {},
})(MuiAccordionSummary) as ComponentType<any>;

const StyledAccordionDetails = withStyles((theme) => ({
  root: {
    padding: theme.spacing(2),
  },
}))(MuiAccordionDetails) as ComponentType<any>;
...
const Fix: FC<FixProps> = ({ id, fix }) => {
  ...
  return (
    <StyledAccordion key={id} className={"bg-gray-300"}>
        ...
        <div className={"flex flex-col"}>
          {fix.recommendation !== 0 && (
            <span className={"text-xs uppercase"}>
              {getNameFromRecommendation(fix.recommendation, i18n)}
            </span>
          )}
          <span className={"font-bold"}>{fix.description}</span>
        </div>
      </StyledAccordionSummary>
      <StyledAccordionDetails className={"rounded-[inherit]"}>
        {steps.length > 0 ? (
...
export const Issue: FC<IssueProps> = function (props) {
  ...
  return (
    <StyledAccordion className={`border-l-4 ${borderColour} bg-gray-200`}>
      <StyledAccordionSummary
      ...
      <StyledAccordionDetails className={"flex flex-col"}>
